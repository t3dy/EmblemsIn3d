...
2. **Reads it.** A 37-stop guided tour covering **all thirty-eight chapters and both
   parts**, from the dark wood to Polia's epitaph, with 253 typed commentary notes under
...
|---|---|---|---|
| **v6** | current | [`/`](https://t3dy.github.io/EmblemsIn3d/) | 10 September 2026. **The monuments stand where the book puts them.** The winged horse, the elephant and the colossus move to the near side of the Great Portal, into the four-square court of thirty paces that chapter III describes and nobody had built — chequered marble laid point to point, two areostyle colonnades at fifteen paces (one standing, one fallen, because the fallen one is the heap Poliphilo climbs to reach the colossus), and plane trees, wild olives, pines and brambles at their feet. The colossus is turned a quarter and laid along the valley, so you meet his bare soles first and come to the head *from thence*, as the text has it. Sited from the 1499 Italian, which settles distances Dallington's English leaves ambiguous. Chapter XI read end to end and Polia dressed out of its blazon — the necklace transcribed stone by stone. 18 of 38 chapters enumerated against the world. Tag `v6`. |
| **v5** | never cut | — | Declared in `DECISIONS.md` and `NEXTSTEPS.md` on 9 September 2026 and never actually released: no badge, no row here, no tag. The site stayed on v4 throughout. Its content — the reading mode, the projected figures, the dream fold, the monuments at true scale — shipped inside v6. Recorded rather than renumbered, so the two documents that name it still make sense. |
| **v4** | superseded | tag `v4` | 8 September 2026. **Roll Up the Dream**, a fifth mode: a ball with Sol and Luna on its faces that eats the garden up the ladder of the metals, over floors littered with nine thousand objects the book names and buildings made of drums, ashlar and voussoirs that settle and topple when undermined. Three stations the coverage ledger found missing: Eleuterylida's banquet, Cupid's *exeres*, the sepulchre of Adonis. The Dream ends Book I where Colonna did. Every mode written up in its own brief (`MODES.md`). Tag `v4`. |
...
| **The Vaults** | Chapter V as a roguelike crawl under the pyramid: a new maze every descent, pillars of four, six and eight sides, unlit vaults in the floor, three everlasting lamps to find and a little wicket to the next depth, with the dragon hunting you through it. Depth is the score. |
...
src/main.js             UI, the tour, the five modes, data loading, the Graphics menu
src/scenes/
