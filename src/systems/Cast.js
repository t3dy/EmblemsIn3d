...
import { isVariant, variantOf } from './AssetVariants.js?v=11';
...
export function makeCast(S) {
  ...
  function figure({ name = '', h = 1, skin = SKIN, robe = null, pose = 'stand', crowned = false,
    ...
    // the projected rung: assembled as `modelled`, then given the painting
    const projectId = (lit && figVariant() === 'projected' && robe != null && !winged && !twoHeaded)
      ? pickCutout(name, true) : null;
    ...
    if (projectId) projectCutout(g, projectId);
    return g;
  }

  // ── Projection: real Renaissance pigment on a modelled body ───────────────
  //
  // Ted, 2026-09-09: "is there any way we can take the Renaissance images that
  // look like nymphs and adapt them into 3d models? I don't like using flat
  // imagines cut from paintings for the nymphs."
  //
  // The card register put genuine period painting into the world, which is the
  // most defensible thing a digital edition can do, and paid for it with a flat
  // plane that swivels to face you. The modelled register gave a real body and
  // paid for it by reading as a mannequin however well it was tuned
  // (HUMANOIDS.md 2a-2e). This takes the half of each that works.
  //
  // WHY IT LANDS IN THE RIGHT PLACES. Both things are a standing figure at the
  // same proportions, seen from the front: the cutouts are cut at one scale so
  // a card already carries each figure's true height, and the modelled build is
  // on a canon. So a planar projection down the view axis puts the painted head
  // over the head and the painted gown over the gown without any per-figure
  // fitting. The paint supplies the SURFACE; the geometry supplies the
  // SILHOUETTE -- which is the whole point, because the silhouette was the part
  // the flat card got wrong.
  //
  // Three deliberate choices:
  //   · NO alphaTest. The card needed alpha to cut the figure out of its
  //     background; here the mesh is already the figure, so an alpha cut would
  //     only chew its edges. Opaque, and the outline comes from the model.
  //   · The UVs are INSET a little. At the very edge of the mesh the projection
  //     samples the painting's transparent border, which fringes; pulling the
  //     mapping in by a few per cent keeps every sample inside the pigment.
  //   · Run AFTER contrapposto, so the mapping follows the pose. A figure that
  //     shifts its weight takes its painted drapery with it.
  //
  // The known limit, stated rather than hidden: this is camera mapping, so the
  // BACK of the figure wears the front's pigment mirrored through. On a draped
  // gown that reads as cloth and nobody notices; it would not survive a figure
  // whose back was meant to differ from its front.
  // WHERE THE PIGMENT ACTUALLY IS on each card. Generated by
  // scripts/cutout_uv_bounds.py from the PNGs' own alpha channels, and also
  // recorded as `card_uv` in src/data/figure_cutouts.json.
  //
  // This table is the whole difference between the projection working and not.
  // The cutouts are centred on a fixed 448x896 card and deliberately NOT scaled
  // to fill it -- they are cut at one scale so the card carries each figure's
  // true height, and a short figure sits lower. paintedFigure wants that. A
  // projection does not: the first attempt mapped the mesh across the WHOLE
  // card, and since a figure occupies only u 0.19-0.81, about 60% of the width,
  // two fifths of every sample landed on the transparent margin, which is
  // black. The nymphs came out with dark gowns and pale stripes.
  const CUTOUT_UV = {
    'chloris':   [0.2255, 0.0034, 0.7723, 0.8426],
    'flora':     [0.2098, 0.0000, 0.7902, 0.9799],
    'grace_1':   [0.2902, 0.0000, 0.7098, 0.9130],
    'grace_2':   [0.2924, 0.0034, 0.7054, 0.8806],
    'grace_3':   [0.2634, 0.0000, 0.7366, 0.9096],
    'mercury':   [0.1897, 0.0034, 0.8103, 0.8505],
    'venus_bot': [0.2120, 0.0034, 0.7857, 0.8627],
  };
  const CARD_PX = [448, 896];          // and the card's own aspect, for isotropy

  const _projMats = new Map();
  function projectCutout(g, id) {
    let mat = _projMats.get(id);
    if (!mat) {
      // white base under an albedo map, or the colour multiplies the pigment
      mat = S.mat({ color: 0xffffff, roughness: 1.0, metalness: 0.0 });
      mat.map = cutoutTexture(id);
      _projMats.set(id, mat);
    }
    ...
    // the figure's own extent, measured in the group's space
    const box = new THREE.Box3();
    const v = new THREE.Vector3();
    const local = (m, i) => v.fromBufferAttribute(m.geometry.attributes.position, i)
      .applyMatrix4(m.matrixWorld).applyMatrix4(inv);
    for (const m of meshes) {
      const pos = m.geometry.attributes.position;
      for (let i = 0; i < pos.count; i++) box.expandByPoint(local(m, i));
    }
    const H = Math.max(1e-6, box.max.y - box.min.y);
    const cx = (box.min.x + box.max.x) / 2;

    // the figure's rectangle on the card, pulled in slightly off the feathered
    // edge the cutting script leaves
    const r = CUTOUT_UV[id] || [0.20, 0.0, 0.80, 0.95];
    const INSET = 0.98;
    const uMid = (r[0] + r[2]) / 2, vMid = (r[1] + r[3]) / 2;
    const u0 = uMid + (r[0] - uMid) * INSET, u1 = uMid + (r[2] - uMid) * INSET;
    const v0 = vMid + (r[1] - vMid) * INSET, v1 = vMid + (r[3] - vMid) * INSET;

    // ISOTROPIC: fit the mesh's HEIGHT to the pigment's height, then use the
    // same physical scale across. A world unit maps to (v1-v0)*896 pixels
    // vertically, so it must map to the same number of pixels horizontally,
    // which is (896/448) as much in u-units as in v-units. Fitting width and
    // height independently is what squashes a narrow figure across a wide card.
    const sV = (v1 - v0) / H;
    const sU = sV * (CARD_PX[1] / CARD_PX[0]);
    const uCtr = (u0 + u1) / 2;
    ...
    for (const m of meshes) {
      ...
      for (let i = 0; i < pos.count; i++) {
        const q = local(m, i);
        uv[i * 2]     = uCtr + (q.x - cx) * sU;
        uv[i * 2 + 1] = v0 + (q.y - box.min.y) * sV;
      }
      // the part geometries are SHARED between figures, so they must be cloned
      // before their UVs are rewritten -- otherwise the first nymph projected
      // would re-map every nymph in the world, and each would wear the last
      // one's pose
  ...
  function nymph({ name = '', robe = 0xb8a0c8, h = 0.95, pose = 'stand',
    ...
    // A garlanded rider keeps her built head for the same reason she does on the
    // card: a Botticelli figure brings her own, and would lose the wreath.
    const projectId = (lit && figVariant() === 'projected' && !garland && cutout !== null)
      ? pickCutout(name, false) : null;
    const g = new THREE.Group();
    const parts = g.userData;
