# Design decisions — the archive, September 2026

*The full text of every directional call, newest first. **`../DECISIONS.md` is the index**: read that first, find the one line you need, and come here for it. Nothing is summarised away — this file is the complete record and the index is only a way in.*

*New decisions are appended to the TOP of this file, and a line is added to the index. See `ENGINEERING.md` §2c for why the two are separate.*

---

## 2026-09-21 — 67. Call 54 is partly reversed: the world shrinks back down, and gaps grow only when a building needs them

*Reverses the SPACING half of call 54 (2026-09-17), keeps the SCALE half. Touches
`scripts/plan_build.py`, `research/plan.json`, `src/scenes/world/cythera.js`, `RETHINK.md`,
`NEXTSTEPS.md`, `research/infill_plan.json`, `research/tickets.json`.*

**The call, Ted's, after walking the deployed true-scale build:** *"I think I've changed my
mind about having things super spread out. I'd like to go back to the relatively smaller
island, although we should move things apart as necessary as we add more buildings. Highest
priority is to make sure all the scenes of the woodcuts have analogs in our virtual world.
And we have a lot of work to do making the buildings beautiful and the gardens filled with
plants that look like the plants named in the novel or our scholarship."*

**What this reverses, and what it does not.** Call 54 chose true scale for two different
things at once: the SIZE of monuments and gardens against the book's own numbers (kept — a
1,139.6 m pyramid base and an 88.8 m green enclosure are still correct and still worth
having), and the DEFAULT GAP of one stadium (185 m) inserted between every precinct the book
does not itself put a distance on (reversed — that default is what made the walk between
buildings "mostly empty spaces," not the buildings' own true size). The two were bundled in
one call and one number (`STADIUM` in `plan_build.py`); they needed to be two calls.

**The new rule for `plan_build.py`'s PRECINCTS table:**

1. **`gap_src=('ours', …)` filler gaps** — the ten precincts whose only reason for 185 m of
   clear ground was "one stadium, no textual basis" (`wood`, `palm_plain`, `valley`,
   `wooded_country`, `cypress_avenue`, `polia_garden`, `triumphs`, `vertumnus`,
   `venus_temple`, `polyandrion`, `shore`) drop to a new default, **`THRESHOLD = 30 m`** — a
   real felt arrival at the next place, not a desert crossing. `default_gap_m` in the
   generated plan reflects this.
2. **Gaps whose `ours` reasoning cites the BOOK'S OWN WORDS for distance**, not just the
   stadium default — `great_oak` ("the one place before the valley where the book insists on
   distance in words rather than numbers") and `three_doors` ("a plentiful seate and pleasant
   Countrey" is a country, not a threshold) — keep a wider **`NARRATIVE_GAP = 75 m`**, since
   they are still carrying a textual claim, just not a numeric one.
3. **`stated` gaps and sizes are untouched** — the pyramid's 1,139.6 m base, the green
   enclosure's 88.8 m, the cypress avenue's own stated 740 m (Dall. pp. 123–124, the single
   explicit inter-station measurement in the whole book) are the book's numbers and stay the
   book's numbers. This call is about invented emptiness, not about the text.
4. **The valley's approach** (10 stadia, the largest single guess in the table, built to let
   the pyramid's spire grow "by little and little") is halved to **5 stadia (925 m)**. The
   reveal is worth keeping — it is exactly the kind of "spectacular experience... Poliphilo
   describing and reacting to" the standing brief asks for — but at 1,850 m it was reveal
   stretched into padding. `DIRECTIONS.md` §5's "nothing approached across open ground with
   the destination in view" is answered by filling the remaining approach with incident
   (trees, mounds, the valley's own build queue items), not by raw distance — that is
   build-queue work, not this decision.
5. **The crossing** (6 stadia, the boat passage timed to a whole chapter of song) drops to
   4 stadia (740 m) for the same reason, proportionally less since a boat crossing is
   supposed to read as a journey rather than as a gap between buildings.

**Cythera itself — "the relatively smaller island," read literally.** The island's stated
diameter (1,400 m, from the book's own circumference and π) is not wrong, but Ted named it
specifically. `src/scenes/world/cythera.js` gets an `ISLAND_SCALE = 0.45` applied uniformly to
every distance derived from the stated diameter — rim, citrus espalier, peristyle, the
theatre and Adonis's fountain all keep their proportions to each other and to the book's own
ratios, only the whole island is built at 45% of the literal-scale figure (~630 m across).
This is a directorial override of a stated number, the same kind of call call 3 of 2026-09-09
made for individual monuments, now made for the one place in the book that is a self-contained
world rather than a station along a road.

**New priority order, replacing the true-scale infill queue as the standing work:**

1. **`research/coverage.json`'s build queue (246 unbuilt, 42 partial features) is the
   priority-one source of truth for what to build next.** "Does every woodcut have an analog"
   is exactly what that ledger already tracks chapter by chapter (`COVERAGE.md`), and it does
   not depend on how the gaps are sized. `/audit-coverage` and `/research-chapter` /
   `/build-feature` are the standing commands for it.
2. **Buildings get a beauty pass** — proportion, material, ornament — using
   `SOURCES.md`'s "Modelling the 3-D assets" table and period architectural knowledge to fill
   what the text alone underdetermines (per the standing brief: *"as the designer you might
   have to go beyond the text... to finish out everything that needs to be built"*).
3. **Gardens get species-true planting**, not stand-in greenery: the named plants
   (`WOODS.md`, the Rhizopoulou ground-flora decision, `decisions/2026-09.md` "the gardens:
   realism was parameters, fidelity was geometry") extended to trees and hedges the same way
   the ground flora already got it.

**What happens to today's true-scale infill tickets.** They are not wrong, they are
re-scoped: `feat-infill-bridge-grove-river`, `feat-infill-orchard-quincunx`,
`feat-infill-jasmine-arbour-reveal` and `bug-itinerary-path-misses-three-precincts` were
written to fill 185 m gaps with content; the gaps they fill are about to be a third that size,
and some of that content (the bridge, the grove, the orchard) is *also* coverage-queue
material, so it is not lost, only smaller and reprioritised behind item 1 above. `RETHINK.md`
gets a note pointing here rather than a rewrite, since its diagnosis (cramped interiors, the
garden trees at the origin, the meadow's cost) is still correct and is not what this call
touches.

**Regenerating the plan is safe under stage 2's own architecture.** Every precinct is already
built inside a `_placeAt` group carrying a computed shift (`shiftOf(key)` / `toWorld(key,x,z)`,
call 60), so re-running `plan_build.py` with smaller numbers moves precincts closer together
without anyone re-typing a single interior coordinate — which is the entire reason that
machinery was built two days ago. Verify by `hpDiag()` before and after (this changes
positions, not triangle count, so the budget should not move) and by walking the shortened
gaps live.

*Settles `question-temple-crown-short-of-the-diameter`. Touches `src/scenes/world/temple.js`.*

Ted, walking the live site: *"we have some places where the roof is too low and is not the
sky"*, and, of how the world should be built, *"as the designer you might have to go beyond
the text of the novel … and make sure that our points of view deliver spectacular experiences
such as Poliphilo is describing and reacting to."*

The height pass (`bug-temple-height-not-from-diameter`) derived the wall from p. 204's
four-sixths-of-the-diameter and the roof from its double-square slope. Both are the book's own
words, and they leave the crown at 11.07 m over a 12.4 m span (0.89 D), so p. 197's universal
rule — *"as much as the diametral line found, so much did it render its height"* — holds only
if the lantern is counted. Standing inside, the room now reads as a real domed hall but not as
the *sky-reaching cupola* the station's own commentary quotes Poliphilo lifting his eyes to.

**The call: p. 197 governs.** Keep p. 204's cornice at 4/6 D (8.267 m) and derive the roof from
the remainder, 12.4 − 8.267 = **D/3 = 4.13 m**, which puts the crown exactly on the diameter.
Both readings are the book's; this is the one that answers the complaint, and it costs one
constant (`DOME_RISE`), so it is cheap to reverse if the slope looks wrong. The dome's courses
are fixed-height cylinders spaced by sine steps, so they must be re-derived in the same pass or
gaps open between them (at a 4.13 rise the lowest courses space out by 0.717 against a course
height of 0.696).

Made by the designer, not asked of Ted, because his standing instruction delegates precisely
this and the ticket's acceptance criterion ("Ted says which reading") is met by that
instruction. Ted can flip it back.
...
toggle on and off. Make sure the interface makes sense."*

Before this, a player's only record of a mode's keys was an auto-fading toast
(`showHint`), except the dragon, which had its own pair of `.ctl-card` toggle panels
(`flight-ctl`, `cam-ctl`, gated behind two `#world-nav` buttons only shown mid-flight), and
roll mode, which carried a permanent nine-clause wall of text under its HUD. Three systems,
one of them one-mode-only, none of them consistent.

**One `Controls` button, in `#world-nav`** — checked live across the mode chooser, Walk
Freely, Roll Up and the Vaults before choosing it, since the brief's assumption that
`#world-nav` was sometimes hidden was wrong: it is the one constant across every mode. The
dragon's two buttons fold into it, so the bar is a button shorter than before. **One
`.ctl-card`**, content swapped per mode from a single `CONTROLS` table in `main.js` rather
than duplicated HTML. Roll mode's footer is now a one-line cue naming the button. The panel
survives a mode's own state changes (roll stages, tour stops, page turns) and only re-renders
when the *mode* changes. The `520px` rule that hid it on mobile entirely is gone; verified at
386×776 and 356×636 in a real narrow viewport.

**The audit that came with it — dispatching keys live and measuring, not reading the code —
found four real, live bugs**, all fixed in the same pass:

1. **Walker bounds never widened for Stage 2.** The box was 2 672 m; the plan is 13 728.8 m.
   22 of 28 stations stood outside it — `0`/`1`–`9` teleported you there correctly, then your
   first step snapped you back thousands of metres. Bounds now come from
   `research/plan.json`'s own extent.
2. **Dragon bounds, same staleness**, plus a 48 m ceiling in front of a 790 m pyramid.
3. **Gallery's Esc was a dead empty block** while the hint promised it closed the gallery.
4. **`ShiftRight` did not dash in roll mode**; `Walker.js`/`DragonFlight.js` both already
   took either shift key.

Also: Poliphilo's Dream had no keyboard path to its own reaction choice — `1`–`4` now pick
it, by index into whatever is actually rendered, so it can't outrun the options on screen.

A second pass caught its own mistake before Ted had to: the panel's measured ceiling read
its own inline offset with `parseFloat(st.style.bottom)`, and `parseFloat('9rem')` is `9`,
not the ~144 px it meant — `getComputedStyle` fixed it (commit `b24b8fc`).

Recorded after the fact as closed tickets, since `research/tickets.json` was mid-edit by a
concurrent research pass when this shipped: `bug-walker-bounds-stale-after-stage-2`,
`bug-dragon-bounds-stale-after-stage-2`, `bug-gallery-esc-does-nothing`,
`bug-roll-shiftright-no-dash`, `debt-dream-mode-no-keyboard-choice`.

---

## 2026-09-20 — 63. Roll mode's camera falls behind the ball's growth, Katamari-style

*Settles `question-roll-camera-curve`. Touches `src/systems/RollUp.js`.*

Ted: *"I want the camera to behave like it does in Katamari Damacy, zooming out when
the ball grows so that you can see the bigger things you are becoming able to roll up."*

The mode has carried two working curves since `debt-roll-wedding-not-a-moment` added the
dials: the original, purely proportional `dist = r * 6` (holds the ball at a constant
fraction of the frame from 0.22 m to 18 m — you can see you're eating bigger things, but
not that you've become bigger), and the sub-proportional `dist = camBase + r * camScale`
that `ROLLMODEPLAN.md` §1.1 asked for, where the camera falls behind the growth and the
ball visibly swells in frame. Ted took the second.

**Default changed**: `tune.camBase` 0 → **1.2**, `tune.camScale` 6 → **2.0**. Concretely,
camera distance goes from 1.3 m → 5.2 m → 13.2 m → 37.2 m at r = 0.22 / 2 / 6 / 18, instead
of the old 1.3 m → 12 m → 36 m → 108 m. The known trade, stated when the dials were built
and accepted here: less of the garden is visible at full size, since the camera pulls back
a third as far. Both curves stay reachable live for comparison
(`window._hp.state.activeScene.roll.tune.camBase = 0; .camScale = 6` for the old one), and
the wheel-zoom clamp (`RollUp.js`, `_onWheel`) already reads off `_camWant()`, so it moves
with whichever curve is active without a separate change.

## 2026-09-20 — 64. No wedding fanfare — the 2026-09-04 silence rule stands

*Settles `question-roll-wedding-fanfare`. No file changed.*

Asked directly whether roll mode's wedding cinematic (reaching 18 m) should get a short
audio cue as the one deliberate exception to the site-wide silence rule (2026-09-04,
"No music, no ambient bed, no sound of any kind... Do not add audio to this project
without Ted asking for it"): **no**. The wedding stays a visual-only cinematic;
`AlchemicalAudio.fanfare('wedding')`'s call site (`src/main.js`, added for
`debt-roll-wedding-not-a-moment`) remains wired to a documented no-op. `RECIPES/verify-live.md`'s
`AudioContext`-proxy silence check needs no exception clause.

---

## 2026-09-20 — 62. `node --check` is not a syntax gate for ES modules

Found by shipping a genuinely broken object literal behind a green check. On node 24,

```js
export const x = { a: { 0x123, b: 1 } };
```

is a syntax error and `node --check` **exits 0**. Every "syntax ok" printed over a file in
`src/` on 2026-09-20 — dozens of them — meant nothing, and one invalid literal went into
`constants.js` and was only caught because the running page stopped building.

`scripts/parsecheck.mjs` replaces it. It *imports* the file, which parses it; the import then
fails to resolve `three`, or to find `document`, which are different errors and are what a pass
looks like. Anything else exits non-zero.

```bash
node scripts/parsecheck.mjs src/**/*.js
```

The general lesson is the one this project keeps relearning in new costumes: **a check you have
not seen fail is not a check.** The first thing to do with a new gate is feed it something
broken.
...
Ted chose the triumphs off `BUILDINGPLAN.md`'s FIRST list. Three separate faults, all the same
shape — a table applied without asking which car it belonged to:

1. **The stones.** `_triumphCar` hardcoded the wheel to emerald and never built the tablets at
   all, so three of the four cars wore Europa's wheels. The book gives every car two stones and
   they are arguments: Scythian emerald and Indian **diamond** (Europa); dark agate and eastern
   **sapphire** sparked with gold (Leda); Ethiopian chrysolite and green Cyprus **heliotrope**,
   the bloodstone (Danaë); Arcadian **asbestos** and Troglodyte **carbuncle** (Bacchus) — the
   stone fire cannot touch, on the car that carries a woman burnt to ash, and the gem the
   ancients thought burned of itself. Now `wheel` and `tablet` in the `TRIUMPHS` table.

2. **The liveries.** `TRIUMPH_LIVERY` was one flat six-entry array applied to every car
   regardless of `t.key`, so Leda's and Danaë's riders wore Europa's blue/crimson/green. The
   book dresses each: Leda's two crimson, two crowfoot-yellow, two violet; Danaë's six **alike**,
   gold-shot dark blue, with no rank at all — the shower of gold is the livery.

3. **The fourth car has no riders.** Cars 1–3 are each given "six young girls"; the fourth car's
   team description has no such sentence, because **the urn is what rides that car**. The world
   gave it six riders copied from the other three.

And the urn itself is built — eight pages of description that were entirely absent. A gold base
with four crimson-aetites eagles standing back to back, wings raised and touching; on their
elbows a lathed vessel of Ethiopian jacinth, crusted with a topaz vine, emerald foliage and
amethyst clusters, two emerald dragons coiled for handles; and in its mouth, fine and holy ash.
Out of it grows the golden vine that **roofs the six-beast team** — the only canopy over any of
the four.
...
*Executes call 54, and it is the decision about HOW.*

Ted, asked how far to take stage 2 and what to do about the pyramid's depth, chose **full stage 2
— everything onto `research/plan.json`** and **build the pyramid square, and move the north.**

**The method is the whole of it.** Stage 1 multiplied every station position by four and then
re-typed a hundred literals inside the builders to match. Stage 2 is 13.7 km and **does not
re-type anything.** Each precinct is built inside a `_placeAt` group carrying a rigid shift that
`scripts/plan_sites.py` computes from the plan; a builder's interior coordinates stay exactly as
they were authored and the precinct arrives on the plan underneath them. Geometry, wall and
circle colliders, walker floors and NPCs all ride the group.

This is the direct answer to `HANDOVER.md` §4.3 — *hand-copied constants do not move when the
table they were copied from moves* — which has now bitten in four distinct costumes:

- the hand-copied spawn coordinate (2026-09-17);
- the meadow's clearance map, still in the original cramped frame and masking bare ground;
- **the dream mode's paths**, likewise, so "Poliphilo's Dream" had been walking the dreamer to a
  quarter of the way to everything it named since stage 1 and nobody had noticed;
- the triumph cars' moving colliders, which push straight onto `walker.colliders` and would have
  left five invisible walls driving round an empty field 4.5 km from the cars.

All four are fixed the same way: **stop copying the number.** `shiftOf(key)` and
`toWorld(key, x, z)` are the only two things anyone needs, and they are generated.

**The pyramid is square now** — 1,139.6 m of base, a 1,110 m plinth 20.7 high, and 1,410 courses
of 0.56 m drawn in 94 merged bands, 789.6 m to the cube and about 965 to the winged nymph. The
4.5 m gable it used to be was a documented lie forced by the Three Doors standing 22 m behind it;
stage 2 moves the doors 4.1 km north and the lie is not needed. The valley's neck is **derived**
from it — `PYRAMID_W / 2 + PYRAMID_CLEAR`, the book's own ten paces of clearance to the rock —
so the gate can never be wider than the valley it shuts.

**Three consequences that were not on the list:**

1. **Merge per precinct, not per world.** `_mergeInto(scene)` traverses, so it folded the whole
   itinerary into a handful of lumps whose bounding spheres are kilometres wide and therefore
   always in the frustum. Each precinct is merged inside its own group first, so frustum culling
   drops whole precincts — the trick the island group has used since it was built, now applied
   to all twenty-three.
2. **The Polia fold is now a no-op, and that is the right answer.** It existed because the
   palace court stood on the ground her arcade needs; at true scale they are 394 m apart. The
   dream no longer has to not add up here, because the plan made room. Left in place, reporting
   0, because stage 3 may move the palace onto the enclosure's north side.
3. **One sward, the size of the plan.** Four hand-sized ground planes became one, read from
   `PLAN_EXTENT`, and with it the whole class of "the walker steps off the world between two
   precincts" goes away. A plane is two triangles whatever its size.

**Not done, and recorded rather than hidden:** the monuments other than the pyramid have not
grown into the room the plan made — Cythera is a 50 m island in a 1,400 m precinct and the
colossus is still a tenth of his sixty paces. The palace does not yet close the green
enclosure's north side, which is what the book says it is. `labyrinth` takes a zero shift and
stays beside the piazza, four kilometres from the polyandrion it belongs to; it needs rebuilding,
not moving, and a shift alone would carry a wrong siting to a new address.

---


## 2026-09-20 — 59. There are two page numberings, ten pages apart; and a page is placed by opening the scan, not by trusting a column

*Closes `bug-plate-page-seq-offset`. Opens `bug-concordance-section-drift` and
`bug-woodcut-catalog-page-jitter`.*

### The shape of the correction: one constant, not a step

**Our translation page = `hp.db` `page_seq` + 10.** `hp.db` counts from `a1r`, the leaf the
text begins on; `translation/en/` counts from the title page, ten pages — five unsigned
preliminary leaves — earlier. The scans `hp1499_pNNN.jpg` are named in `hp.db`'s numbering.

The ticket left open which shape the correction would take, and named a step function at the
Book I/II seam as the likeliest candidate. **It is not a step.** Eight scan/page pairs were
measured across chapters I, XVII, XXI, XXIV, XXVI, XXXI and XXXIII, and the offset is +10 at
the first woodcut in the book (`p4` = our p.14, the Hercynian forest) and +10 at the last page
checkable, with no break at the seam — `p363` = our p.373 is still chapter XXIV in Book I,
`p387` = our p.397 is already chapter XXVI in Book II, both +10. The ticket's own evidence
carried one "+11" reading at `p435`; that page is our p.445, and the datapoint was a slip.

Corroboration that needs no scan: the only four pages `translation/en/` has no file for — 55,
90, 195, 380 — are `page_seq` 45, 80, 185, 370, and three of those are exactly the book's
full-page woodcuts, which have no body text to translate.

### The rule this produced: a derived column is not an observation

The constant alone did **not** satisfy the ticket's acceptance, and the reason is the part
worth keeping. A second, smaller error rides on top of it: `woodcut_catalog.page_seq` is
itself only good to about ±2 pages, because those rows were never observed — they were
**subject-matched** by an LLM against a `woodcuts` table whose own quire model is wrong.
`woodcut_catalog.link_basis` admits it in words: #153's reads *"subject match on 1499 page 389:
shared terms drags, polia, poliphilus, prostrate, sanctuary"*. Open `hp1499_p389.jpg` and it is
unbroken text, signed "B ii", with no woodcut on it at all; the dragging plate is on `p387`,
above the whole of our p.397. Three plates were corrected by opening the page, and only #153
changed chapter — but #153 was one of the two the acceptance named.

The same lesson, larger, in `page_concordance`: its `section` boundaries agree exactly with
the plates through `PROCESSION` and then drift progressively early, until `BOOK_II_POLIA` is
marked as starting at `page_seq` 271 when Book II opens at 371 — **116 pages out**. Its
signature labels are wrong in the same region (it calls `p387` "C6r"; the leaf is plainly
signed "B"), and it stops at 448 when the book runs to at least 457.

So: **`hp.db` is authoritative for what the book contains and unreliable for where it sits.**
Rule 3 of `CLAUDE.md` says `woodcut_catalog` and `folio_descriptions` outrank your mental
image; this is the boundary of that. For *subject* they do. For *page*, open
`site/images/woodcuts_1499/hp1499_pNNN.jpg` and read the Italian. Do not infer a page from
surrounding text, from the `woodcuts` table, or from a section boundary — all three were
already wrong here. The corpus is read-only, so corrections live in `PLATE_PAGE_FIXES` beside
`plates()` in `scripts/coverage_seed.py`, each carrying the line of Italian that identifies it,
and **confirmations are recorded too** (`PLATE_PAGE_CONFIRMED`) — otherwise there is no way to
tell a page nobody checked from a page somebody checked and left alone.

`check_plates()` asserts sixteen plate→chapter pairs on every seed and aborts the run if any
moves. Reinstating the bug in-process makes it fire, reproducing the ticket's wrong-attribution
list exactly, so the assertion is known not to be vacuous.

---

## 2026-09-20 — 58. Roll mode's HUD says where you are on the ladder; the wedding takes two seconds; the fanfare stays silent

*Closes `debt-roll-size-gauge`, `debt-roll-stage-countdown`, `debt-roll-wedding-not-a-moment`.
Opens `question-roll-wedding-fanfare`. Half-answers `question-roll-camera-curve` by giving it
its dials.*

Four calls were made while building these, and three of them are the sort that get
re-litigated if they are not written down.

### The gauge's ruler is linear, and that is a choice with a cost

The bar fills 0 → 18 m with the seven signs at their own `at` values. The first draft used a
**cube-root** axis, on the argument that the ball grows volumetrically (`ROLLING.md` §1) so
the rungs should be spaced by the work each costs — 0, 32, 42, 52, 63, 74, 84 %, which is
beautifully legible.

It was wrong, and it was thrown away. The ticket's complaint is that *nothing shows how far
18 m is*; a curved ruler answers a different question and **flatters**. The cube root put the
ball 65 % along the bar at 5 m of 18, and 23 % along it before it had eaten anything at all.
Linear in metres is the honest ruler: 5 m reads 27.78 %.

The cost is real and is accepted: Saturn, Jupiter, Mars and Venus all fall inside the first
2.6 m of 18, so four of the seven signs crowd the left-hand seventh of the bar. The bar is
therefore drawn as wide as the HUD allows and the signs as small as they can be read (measured
on the page: no two glyphs overlap at 1024 px). **That crowding is not a defect of the
display. It is the shape of the ladder**, which was deliberately stretched on 2026-09-09 so
that each stage is longer than the last and the gold is genuinely earned.

### The wedding is silent, and the fanfare is Ted's to ask for

`debt-roll-wedding-not-a-moment` asked, in its acceptance, for "a fanfare through
AlchemicalAudio". **It was refused**, because the call of 2026-09-04 (evening, final) is Ted's
and a ticket does not outrank it: *"No music, no ambient bed, no sound of any kind — not
during the guided tours, not in Poliphilo's Dream, not in the Atalanta worlds, not on any
page… Do not add audio to this project without Ted asking for it."*

What was built is the **call site and nothing else**: `AlchemicalAudio.fanfare('wedding')`, a
documented no-op on the stub. Nothing under `src/` constructs an `AudioContext`, proved on the
page by proxying `window.AudioContext` and `webkitAudioContext` and forcing the wedding twice
— counter 0 both times, which is the check `RECIPES/verify-live.md` prescribes. If the one
cue is ever wanted it is one function body, and the question is filed as
`question-roll-wedding-fanfare` rather than decided by a builder.

### The camera keeps its feel; it only gains dials

`tune.camBase` and `tune.camScale` exist now and are read at every use site, so the plan's
sub-proportional curve can be flipped on mid-roll (`camBase 1.2, camScale 2.0`) against
today's proportional one (`0, 6`). **The defaults are unchanged**, and were checked to be the
same numbers rather than assumed to be: `cam.dist` on start is exactly `r × 6`, and the two
clamps that were written as bare radii are ratios of the wanted distance which come out at
`2.4r`/`9r` and `2r`/`14r` as before. The call itself stays `question-roll-camera-curve`, and
stays Ted's.

### A layout bug that had been shipping, fixed in the JS

`setHidden(el, hidden, shown = 'flex')` forces `display: flex` on whatever it reveals, so
`#roll-hud` had always been laid out as a flex **row**: the name, the readout and the controls
side by side, the HUD measuring 1167 px on a 1024 px viewport and starting at x = −71, with
the name off the left edge. Its CSS was written for stacked, centred children. It is revealed
with `'block'` now. Per `CLAUDE.md` — never fix a layout bug in CSS alone when the JS can
enforce it; here the JS was the cause.

### One note on how this was built, because it will recur

`src/main.js` and `src/index.html` were being written **at the same time** by the Stage 2
precinct pass, which is the case `ORCHESTRATION.md` warns about (one writer per file). Rather
than stage its unfinished work — the tree would not even build for part of the session,
`_buildCypressAvenue` being called before it existed — this pass verified against a **detached
worktree at HEAD** carrying only its own four files, and staged those four as blobs so the
other pass's edits to the two shared files stayed uncommitted and untouched. The consequence
worth knowing: `RollUp.js?v=11` **could not be bumped**, because its importers
(`scenes/HPWorldScene.js`, `scenes/world/rollup.js`) were mid-edit. Everything player-visible
was therefore kept in `main.js`, which is bumped, and `window.hpRollAgain` falls back to a
full rebuild if it finds a RollUp without `restart`. Whoever next bumps `RollUp.js` must bump
it in **three** importers now: `main.js` imports `METALS` and `WEDDING` from it.

---

## 2026-09-20 — 57. The roll-mode build that landed in the Atalanta repo is abandoned, not ported; its page stays up

*Ticket `infra-wrong-repo-preflight`. Opens `question-roll-camera-curve`, which is Ted's.*

On 2026-09-20 the "Entry point for next session" block at the foot of `HANDOVER_ROLLMODE.md`
was pasted into a session opened in `C:\Dev\EMBLEMSIN3D` — `github.com/t3dy/emblems-in-3d`,
the live *Atalanta Fugiens* project — instead of `C:\Dev\HPin3D`,
`github.com/t3dy/EmblemsIn3d`. The block named every file by relative path and named no
repository, and the two repository names differ only in case and hyphens.

**The session did not stop when the files were absent.** It rebuilt this project's roll mode
from the *prose* of `ROLLMODEPLAN.md`: `src/roll-up.html`, a 282-line `RollUp.js` and a
487-line `RollUpHUD.js`, committed in `4de9021`, `7b82414`, `a009bda` and published to
`t3dy.github.io/emblems-in-3d/roll-up.html`.

### The call: abandon it. Nothing is ported.

Not because it is the wrong repo — that alone would argue for moving it — but because it is
**wrong on its own terms**, in three ways that cannot be patched out:

1. It stages the work as **NIGREDO → ALBEDO → CITRINITAS → RUBEDO**. This project climbs the
   **seven metals**, Saturn to Sol, taken from `hp.db.alchemical_symbols` and argued for in
   the header of `src/systems/RollUp.js` — the ball is a *rebis*, gold on one face and silver
   on the other, and the wedding at 18 m is the conjunction of the two. The colour-stages are
   not cited from anything (rule 2), and they contradict §4.1 of the document the mock was
   built from.
2. It has **no world**: no census, no rollables, no creatures, no crust, no masonry. Its
   "collection counter" counts nothing — its own HUD concedes the `+1` is "ready for item
   system" and there is no item system. The ball grows from distance travelled across an
   empty 200 × 400 m plane.
3. Its physics ignore the growth curve **measured** on 2026-09-09 — `packing` 0.38, `bite`
   0.48, arrived at by a headless sweep after a guessed pair stalled the ball at 0.97 m
   (`ROLLING.md` §1).
...
Ted, shown it: *leave it for now*. So `t3dy.github.io/emblems-in-3d/roll-up.html` continues
to serve, and the two stray HP planning documents stay in that repo's root. Rule 4 cuts the
other way here in principle — a Hypnerotomachia demo does not belong on the Atalanta site —
but unpublishing is a public-facing change and it is his to make. Recorded so that a later
session finding an HP roll-up on the Atalanta site knows it is a known artefact and not a
fresh leak, and does not quietly "fix" it.

### What the failure bought

The correction was worth more than the code would have been. Re-reading `RollUp.js`,
`main.js` and `index.html` to judge what could be ported found that the handover's own
baseline was **wrong in four places**: it claimed there was no HUD, that the camera did not
scale, that the run just stopped with no ending, and that there was no sound system. All four
were false — `#roll-hud` shows Size / Things / Metal (`src/index.html:1199`), the camera holds
`r × 6` clamped to `[r × 2.4, r × 9]` (`RollUp.js:416`, `:691`), `onWedding` fills a score
card (`main.js:1389`), and `src/systems/AlchemicalAudio.js` exists and roll mode simply never
calls it. **A session obeying that brief in the right repo would still have rebuilt what was
already there.** `ROLLMODEPLAN.md`'s "Current state:" lines carry the same rot and are now
marked stale at the top of the file.

So Phase 1 is three items, not five: the **size gauge** with the seven signs on it, the
**countdown to the next metal**, and the **wedding as a moment** rather than a card — tickets
`debt-roll-size-gauge`, `debt-roll-stage-countdown`, `debt-roll-wedding-not-a-moment`.

The camera formula is **not** among them. `1.2 + r × 2.0` is sub-proportional where today's
curve is proportional, so the ball would swell in frame as it grows — the Katamari reading —
but it would also cut the view at full size to a third of what it is now, at the moment the
garden has finally opened. That is a change of feel to a mode Ted has played, so it is his:
`question-roll-camera-curve`, with `tune.camBase` / `tune.camScale` to be added so he can flip
between the two mid-roll.

### Baked in, so it cannot recur

- All three agent briefs in `.claude/agents/` now open with a **Step 0**: run `git remote -v`,
  stop if it says `emblems-in-3d`, and confirm the named files exist before planning.
- The rule that actually matters is stated with it: **a file your brief names, which is not
  there, is a contradiction to report — never a blank page to fill.** The wrong directory was
  the accident; filling in the blanks from a description is the failure.
- `CLAUDE.md` rule 4 and `ROUTER.md` rule 4 carry the two repository names side by side.
- Entry-point paste-blocks now name the absolute repo path and the expected remote.

---

## 2026-09-17 — 56. Stage 1 of the true-scale plan built: SPREAD = 4

*Ticket `plan-resite-precincts-true-scale`. Follows call 54, which chose true scale, staged,
over a uniform 1:2 or true-distances-with-capped-monuments.*

**What shipped.** Every station in `HP_STATIONS` (`constants.js`) has its `pos` and `look`
multiplied by 4 about the world origin (0, 0); `radius` is untouched. Every precinct builder's
own anchor constant — `_buildCourt`'s `CX,CZ`, `_buildGreatPortal`'s `Z`, `_buildColossalHorse`'s
`HX,HZ`, the `_placeAt`/`_placeColossus` call-site offsets, the default centres on
`_buildVenusTemple`/`_buildPriapusRite`/`_buildWaterLabyrinth`/`_buildBookTwo`, `PIAZZA.z0` — is
multiplied by 4 too, which is the same move: a builder's whole subtree translates by 3x its own
anchor, so the anchor's new absolute position is exactly 4x the old one. `WOOD` and the "fields"
precinct (genuine SIZES, not points) translate instead, keeping their depth/width and moving
their centre by 4x. Radii and rings that are DISTANCES against other now-moved things scale too:
`_foldPoliaCourt`'s ring (9/42 -> 36/168), `Litter.js`'s mainland/island split (-95 -> -380) and
its dust/meadow bounding boxes, the Walker and DragonFlight bounds. Extent-defining ground —
the 130x130 holed sward and its crosspaths, the 300x172 and 280x70 approach meadows, the two
walker fences, the 170x185 sea plane and 130x4.5 sand strip, `Meadow.js`'s default grass bounds —
was grown x4 as well as repositioned, because it is the floor under a world that just spread out
and a plane that only translated would leave the walker standing on nothing between precincts.

**The overlap the ticket was opened for is gone.** The station-spacing script (this ticket's
own acceptance line) reported 9 overlapping pairs before, worst by 12.8 m; after, **1 pair at
-0.1 m** (`chess`/`artificial`, a rounding-scale clip from the plain x4), fixed with a 2.4 m
nudge to `artificial`'s `pos` — not a rule change. Re-run: **0 overlapping pairs.**

**A hardcoded default spawn was found broken by this, live.** `HPWorldScene`'s constructor set
the no-station default spawn to a literal `(0, 0, 448)` — a copy of the OLD `plain` station's
`pos`, not a read of `HP_STATIONS`. Scaling the table did not move it, so the walker woke up 4x
short of the real plain until this was caught by teleporting there and reading the position back
from the live page (rule 1). Two more hand-copied spawns (`hpRoll`'s `(7, -7)`, `startDream`'s
`(0, 0, 49.5)`) were found the same way, by grep, and moved x4 to match.

**What this is not.** `research/plan.json` holds the actual true-scale plan — 23 precincts,
13,729 m along the itinerary. Four is roughly 40% of the way there, and stages 2-4 (grow the
monuments, fill the new room with the 251 unbuilt features) are still open on the ticket. The
world is now **deliberately sparser** — ground between the spread-out monuments with nothing
on it yet — which is the intended intermediate state DECISIONS.md 54 already named, not a
regression to compensate for by shrinking the spread.

**What was NOT re-sited, and is now misplaced rather than merely cramped.** Decorative,
unstationed scatter with no clean per-object anchor — the bird perch/ring tables and turf-seat
sitings in `nature.js`, the ambient pollen motes — was left at its old coordinates. It is
harmless (no collider blocks a path, nothing is unreachable) but it now sits in what is often
empty ground between precincts that used to be its neighbour. Flagged for a later pass, not
fixed here: relocating eighteen hand-placed bird perches and five flight rings each to whichever
precinct they belonged to, individually, was judged not worth doing in the same commit as the
mechanical spread.

**Measured, per rule 7.** `hpDiag()` at the `elephant` station (piazza, most-crowded before):
before **5,737 draw calls**, after **~3,450** — a decrease, because standing in the same spot
now puts fewer things in the same view frustum once they are spread apart. This is the pattern
call 54 predicted ("draw calls are a property of the VIEW") and it means the regression budget
was never in danger.
...
**The call, Ted's:** *"make sure we build everything that's in the novel."* Asked whether Book
II — chapters XXV–XXXVIII, Polia's own telling of her history in Treviso — should get real
ground or stay a single station with commentary, he chose **real ground**.

**What that means.** The world gains a second setting. Today the fourteen chapters of Book II
are answered by one station (`book_two` at (44, 32), `tombs.js` `_buildBookTwo`) holding
material from chapters XXVI–XXXV only. Under this call, Treviso is a place you go: the temple
of Diana with its Vestal rule, Polia's own bed-chamber, the streets and the river Sile, the
plague, the vision of the torn lovers, the temple of Venus where the story is told.

**The reading rule that follows, and it is the one that matters.** A told room is geometry. The
researchers enumerating Book II are instructed that a room, a street, a temple, a river or a bed
counts as a feature even though Poliphilo never walks into it, because in this world he will.
Book II had been half-invisible to the ledger for exactly the reason chapter V's vaults were:
not a missing plate this time but a **missing kind** — nobody had decided that narrated places
were buildable, so nobody enumerated them as places.

**What it costs.** It roughly doubles the buildable world, and Book II is the half of the novel
nobody illustrates. It also, on the first chapter read under it (XXV, enumerated 2026-09-17),
immediately turned up the scene the whole romance is built on and which the world does not have
anywhere: **Polia at her window combing her golden hair, and Poliphilo passing below struck like
a lightning-split oak.** The existing `book_two` precinct jumps straight past it to the Temple
of Diana.

---

## 2026-09-17 — 54. The world is rebuilt at the book's own scale. This reverses call 3 of 2026-09-09

**The call, Ted's:** *"I feel like we still don't have all the structures that Poliphilo visits
in the novel, and our dream garden is still too cramped. I'd like to spread everything out and
make sure we build everything that's in the novel."* Offered true scale, true-distances-with-
capped-monuments, or a uniform 1 : 2, he chose **true scale, 1 : 1, staged**.

**This reverses call 3 of 2026-09-09** — *"rescale the monuments where they stand; leave the
ground plan alone"* — which declined `DIMENSIONS.md` §5's 1 : 8 plan. It does not reverse the
reasoning behind it: that call's own "known constraint, to be solved and not designed around"
was that the cliffs converge to 44 m against a portal the book gives 1 140, so *"the book's own
absolute — no man could go further forward or backe againe — quietly stops being true."* That
constraint is now the brief rather than the caveat.

**Nothing here fights the 2026-09-09 statement of purpose.** *"I don't want to sacrifice space
for speed just to get the player around in the world faster… we can always give the player the
ability to run or fly or teleport."* Call 1 of that day — space is never traded for speed,
impatience is answered with options — is the licence for this one. Run, fly and teleport already
exist; what they carry the player across is about to become worth crossing.

**The measurements that made the case, taken 2026-09-17 and not estimated:**

| | |
|---|---|
| the mainland, all 18 of its stations | **77 × 105 m** |
| the green enclosure before the palace, on its own (Dall. p. 124) | **88.8 m square** |
| pairs of stations whose radii physically overlap | **nine** |
| worst of them: the gardens of glass and silk inside the chess ground | **12.8 m of overlap** |
| next: the colossus inside the water labyrinth | **11.8 m** |
| the monuments' built scale against the book (`DIMENSIONS.md`) | **~1 : 60** |
| the gardens' built scale against the book | **~1 : 12** |

The last two lines are the whole diagnosis. **The compression is not uniform**, so the world
does not read as a small world — it reads as a crowded one, which is a different and worse
thing. A station standing inside another station's radius is not a tuning problem.

**Staged, and in this order**, because a 1 139 m pyramid is not a first move:

1. **The plan as data first** — one siting table, every separation carrying its source and
   marked *stated* or *ours*. Nothing moves until the table says where it lands, so that no
   precinct is moved twice.
2. **Precincts translate whole.** `_placeAt` (`world/materials.js`) already reroutes `this.scene`
   and patches `_wallCol`/`_circleCol` so baked-in world coordinates, plaques and colliders all
   travel together. A precinct moves by wrapping its builder in an offset, not by editing a
   thousand coordinates.
3. **Then the monuments grow** into the room the plan has made, feature by feature off the build
   queue.

**The blocker everyone expects here is already gone, and the document that names it is stale.**
`DIRECTIONS.md` §6A says `sun.shadow.camera` is ±58 with `far = 130`, so nothing outside that box
casts and a bigger world silently loses its shadows. That was true when it was written and was
fixed on **2026-09-08**: `src/shaders/HPStyles.js` now builds a `trackedSun` whose shadow box
**follows the walker** at ±118 m with `far = 700`, snapped to the shadow map's own texel grid in
light space so the edges do not crawl. A tracking box does not care how big the world is. §6A of
`DIRECTIONS.md` should be read as history, not as a queue item — and this entry said otherwise for
an hour on the day it was written, which is the ordinary way a stale document propagates.

**The real constraint at true scale is the opposite one**, and it is stated in
`HPStyles.js`'s own comment: the frustum stays *modest* precisely because it moves, since a fixed
box big enough for the new world would put a metre of world in every shadow texel. So the thing
to watch as precincts spread is not the frustum's size but whether 236 m of tracked box still
covers the largest single object standing near the walker — and at true scale the pyramid is
1 140 m wide. That is a real question, it is not the one §6A asked, and it cannot be answered
before the pyramid grows.

---

## 2026-09-13 — 53. The gardens of glass and silk flank the Queen's palace, at half the book's compass

**The call:** the two artificial gardens stand against the side walls of Eleuterylida's palace
— glass on the south, silk on the north — each about half the area the book gives it, rather
than side by side somewhere with more room.

**Why beside the palace, and which side is which.** The book is specific, and the old siting
had never been read against it. Our translation p. 127: the silk garden is *"adjoining the right
wall of the proud, great and royal palace"*. Dallington p. 175: the glass garden is *"vppon the
lefte side of the incomparable pallace"*. Left and right depend on where Poliphilo is facing, and
p. 174 answers that too — he has just *"turned mee about towardes the conspicuous Poarch, to
beholde diligently the artificious Pallaice"*, so he faces its east front, and left is south.

**Why half-size, and why that is the right compromise.** p. 124 makes each garden *"as great as
that where the majestic residence stood"* — the palace's own plot, 16 × 12 m here. A top-down
height render of the palace side found about 8 × 5.5 m of open ground on each flank: the doors
wall and its canopy close the south strip, the Temple of Venus closes the north. The two ways to
get full size were to move a finished set-piece or to put the gardens somewhere else. The first
breaks built work; the second breaks the one thing the text states exactly, which is that each
garden adjoins a wall of the palace. **Relation was kept and area gave way**, and the code says
so in its header rather than leaving a reader to discover it.

**The layout is the book's too.** The old build was two free-standing circles. The text describes
raised beds *"cleaving to the walls"* with box and cypress *"alternated"*, bed faces of gilded glass
in one garden and tapestry in the other, a green silk-pile ground, and in the middle of the silk
garden *"a round enclosure, with a raised dome of little rods of gold"* covered in roses over seats
of ruddy jasper. All of that is now built. The pearl-crusted walls with gold-veined ivy and the
silk ring-doves are not.

**What reading the pages turned up on the way.** Both gardens, and the three doors, are **chapter
X** by our translation's own page headings (pp. 119–127, and the chapter's rubric on p. 117 lists
the doors). The tour tagged all three stops XII–XIII. Retagged; chapter XII — the rest of
Polia's blazon and Poliphilo following her — now resolves to Polia's stop.

This supersedes the holding position of 2026-09-09 (call 51's note on the gardens) and closes
`bug-artificial-gardens-wrong-side-of-portal`. It does **not** close `bug-court-has-no-room-left`,
whose real cause turned out to be scale: the court is built 15 m across and the book gives it 41.

## 2026-09-10 — Release Version 6

*Live at https://t3dy.github.io/EmblemsIn3d/ at `main.js?v=385`. Tag `v6`.*

**First, the numbering, because it was a fair question.** `DECISIONS.md` and `NEXTSTEPS.md`
both say "Version 5 shipped" on 2026-09-09. It was not shipped. Nothing was cut: the landing
page badge still read **v4**, the README's Versions table still said v4 was current, and there
is no `v5` tag in the repository. It was a narrative marker written in the middle of a very
long day — the day went on for another fifty `?v=` bumps afterwards — and the public site sat
on v4 from 8 September until today.

So this release is **v6**, not v5, and it carries v5's content inside it. Renumbering it to v5
would have contradicted two documents that name a v5; instead the README now has a `v5` row
that says *never cut*, with the reason. There is no retroactive `v5` tag, because there was no
moment that deserved one.

**What is in it.**

The headline is chapter III's **piazza**, and it is the largest single piece of architecture
in the book that had never been built. The winged horse, the elephant and the colossus all
stood *past* the Great Portal while the book and our own tour both put them *before* it, so
walking the tour in its own order crossed the gate three times where the book crosses it once.
Ted's call on 2026-09-09 (call 51) was to move the monuments, and moving them unblocked the
court itself: forty-four metres square, filling the valley's neck and stopping dead against
the porch, paved with a diagonal lozenge chequer in four marbles, broken and overgrown; two
areostyle colonnades at fifteen paces, the east one standing with what is left of its
epistyle, the west one **fallen** — because the heap Poliphilo climbs to reach the colossus
*is* that row; and the plane trees, wild olives, pines and pricking brambles the 1499 plants
at their feet.

The colossus was **turned a quarter** and laid along the valley. He is thirty-two metres and
the neck is forty-four, so lying across it he could only fit by crossing the whole floor. The
turn also fixed an approach that had been backwards since he was built: the book meets *"the
feete thereof bare, and their soles hollowe"* and only *"from thence"* comes to the head.

**The 1499 Italian settled what the English could not.** Dallington's *"in the passage
towardes the Porche, some tenne paces"* is ambiguous in English and had been read as ten paces
*from the porch*; the Italian says *"dal'initio intro verso la porta x passi"* — from the
piazza's beginning, inward toward the gate. `DIMENSIONS.md` was corrected. That is now the
standing rule: go to the Italian whenever a Dallington measurement has to become a coordinate.

**Chapter XI** was read end to end. It has no plate and it still looked covered, because two
of chapter X's tour stops had been filed under it — rule 6's blind spot one level up, tour-
driven rather than plate-driven. What is actually in it is the meeting with the torch-bearing
nymph and five pages of blazon. Polia is now dressed out of it: the duck's-neck green gown,
the girdle of Cytherea, three pearl pins a side, the violet wreath, hair below the knees, and
a necklace **transcribed stone by stone** in the book's own order. Eighteen of the thirty-eight
chapters are now enumerated against the world, up from seventeen.

**Two bugs that were nobody's ticket** were found and fixed on the way. Switching to the
woodcut register had been blanking the entire page — `_buildAdonis` guarded a point light with
`if (S.pointLight)`, which asks whether the style *has* the method, and the woodcut style's
implementation returns null. And twenty-two plaque subtitles across the world were being
clipped at both ends, because `_fitFont` shrinks to a nine-pixel floor and then gives up;
they break onto two lines now.

**Measured.** Draw calls at the valley 456 → 465, at the portal 460 → 393, at Polia 236 → 237.
The world is 5 747 meshes and 3.16 M triangles. All twenty-eight stations, all five modes and
both registers were walked on the deployed page with no console error.

**What it does not have**, so the next reader is not surprised: the festival on the plain that
Polia parts from is enumerated and unbuilt; the gardens of glass and silk are still on the
wrong side of the Great Portal, moved down the valley as a holding position while
`bug-court-has-no-room-left` blocks the only ground that would take them; and the colonnade
capitals are plain drums where Dallington gives them waved shell-work.

## 2026-09-09 (later still) — 51. The piazza moves. The monuments stand before the porch.

**Ted's call, asked and answered:** *move the piazza north into the approach* — not move the
portal, and not record the triple crossing as a deliberate compression.

Chapters III and IV put the winged horse, the elephant and the colossus in a court **before**
the great porch, and the tour has always agreed — valley → horse → colossus → elephant →
portal → three_doors. The ground plan did not: all three stood past the gate, so walking the
tour in its own order crossed the Great Portal three times where the book crosses it once.
Nobody had ever chosen that; the piazza went on the garden side because that was where there
was room, and the tour order was written from the book afterwards.

**What the sources settled, and what they overturned.**

The 1499 sites the horse to the metre, and this had been read wrongly:

> *Sopra di questa piacia, dal'initio intro verso la porta x passi, vidi uno prodigioso
> caballo* — l. 1255

Upon this piazza, **from its beginning inward toward the gate, ten paces**. `DIMENSIONS.md`
had that line as "the winged horse, into the court **from the porch**", which puts him at the
far end from where the book puts him. Corrected there, and the horse stands at z 55.6 — ten
paces (14.8 m) in from the court's mouth at z 70.4.

The elephant follows from *non troppo distante dal magno caballo, **ad libella*** (l. 1385),
which Dallington gives as "not farre distant from the horse straight forward" (p. 46), so he
stands on the horse's own x, 13.6 m nearer the gate.

The colossus was **turned a quarter**, and the turn is a source claim, not a fit. He is 32 m
long and the valley's neck is 44 m wide — the width of the piazza itself — so lying east–west
he could only fit by crossing the whole floor. Lying along the valley he lies down one side of
it, which is where the book has him: Poliphilo leaves the paved court and climbs *"vp vppon a
heape of ruinated, broken and downe-fallen marbles"* to reach him (p. 44). And the turn fixes
the approach, which the old placement had backwards — *"I came to a vast and wonderfull large
Colose, **the feete thereof bare**, and their soles hollowe … **From thence** with horror I
came to looke vpon the head."* Feet first, then the head. Now true.

**Built with it, because the call unblocked them:** the four-square court itself (thirty
paces, 44.4 m, filling the valley neck and stopping against the porch), both areostyle
colonnades — east standing with what is left of its epistyle, **west fallen**, because the
heap Poliphilo climbs *is* that row — and the plane trees, wild olives, pines and brambles at
their feet.

**Two things this pass moved that were not on anyone's list.**

1. **The gardens of glass and silk were on the wrong side of the gate too**, in the other
   direction: chapters XII–XIII happen after Eleuterylida's banquet, and they stood 26 m
   *before* the portal, on the exact ground the colossus and the horse now occupy. They are
   pushed 30 m down the valley as a **holding position**, not a fix. A clash sweep of the
   whole palace side found exactly one placement with no wall and no collider, at x = −46,
   past the Temple of Venus — the same shortage `bug-court-has-no-room-left` reports. Ticket
   `bug-artificial-gardens-wrong-side-of-portal`.

2. **A new mechanism: `_placeAt(cx, cz, turn, build)`.** Several builders in this world have
   their coordinates baked in, and cannot be moved by setting a group's position — `_plaque`,
   `_frieze`, `_wallCol` and `_circleCol` all speak world coordinates, so the elephant's Greek
   plaques and every one of the colossus's wall colliders would have stayed behind at the
   origin. `_placeAt` reroutes `this.scene` (the trick `_buildCytheraIsle` already used) and
   patches the two collider registrars. **`turn` is in whole quarter turns and anything else
   throws** — a right angle is the only rotation that maps an axis-aligned rectangle onto
   another axis-aligned rectangle, and that is what keeps the AABB colliders exact.

**Measured, not asserted.** The tour's own station order crosses the portal plane **once**
(was three). A walk-north probe at 23 values of x gets past the gate at x = 0 and nowhere
else, so the portal still closes the valley. A walk-south probe enters the colossus by the
mouth and ends 14.8 m inside the body, so the interior survived the turn. Draw calls: valley
456 → 465, portal 460 → **393** — the portal view got cheaper, because the elephant and both
colossi are no longer standing behind the gate.

## 2026-09-09 (later still) — The nymphs get bodies, and rule 7 gets a reading

*Ted: "is there any way we can take the Renaissance images that look like nymphs and adapt
them into 3d models? I don't like using flat imagines cut from paintings for the nymphs."*

**The call: the figures are PROJECTED by default — real geometry wearing real Botticelli —
and the flat card drops to being one rung of five.**

`HUMANOIDS.md` had recommended the opposite. Its Option D said keep the painted card, on the
good argument that a cut-out puts *genuine period images* in front of the reader instead of
our guess at a period body, which is the most defensible thing a digital edition can do. That
argument survives; what fails is the assumption that the pigment and the flat plane come as a
pair. They do not. Project the cut-out onto the modelled build and the paint supplies the
SURFACE while the geometry supplies the SILHOUETTE — and the silhouette was the only part
the card ever got wrong.

**It works without per-figure fitting** because both things are a standing figure at the same
proportions seen from the front, so a planar projection down the view axis lands the painted
head on the head. It runs after `contrapposto`, so the mapping follows the pose.

**And rule 7 gets its first real reading.** Defaulting to the projection costs +48% meshes and
+54% triangles against the card, which is over the 25% gate. Put to Ted with the numbers, he
answered: *"we have the different graphical options menu for a reason try them all out ...
don't be silly. bling us out."* So: **the 25% gate stops a regression being slipped in
unnoticed; it is not a veto on a deliberate, measured, reversible choice with a menu behind
it.** The cost is not the projection's — a card is one mesh and any figure is a dozen, and the
projection is in fact *cheaper* than `painted`, the other assembled rung, by 331 meshes at
identical triangles, because one shared painted material lets the compiler merge parts it
could not merge before.

| | meshes | triangles | shadow casters |
|---|---|---|---|
| `card` | 3 819 | 2 201 042 | 1 296 |
| `painted` | 5 952 | 3 101 652 | 3 763 |
| **`projected`** | **5 621** | **3 101 652** | **3 432** |

*Corrected 2026-09-09. The first version of this table used a `card` reading of 3 788 /
2 018 782 taken BEFORE Polia's arcade shipped, so it compared two changes at once and reported
+54% triangles. Re-measured at the same version as the other two rungs, card to projected is
**+47% meshes and +41% triangles**. The conclusion — over rule 7's gate, and taken anyway —
is unchanged.*

**The limit, stated rather than hidden:** this is camera mapping, so the BACK of a figure
wears the front's pigment mirrored through. On a draped gown that reads as cloth. It would not
survive a figure whose back was meant to differ from her front.

---

## 2026-09-09 (later still) — The dream does not have to add up

*Ted, mid-build, after two attempts to fit Polia's garden onto the island had failed on
physics: "remember that this is a dream garden. as events happen in the book during the tour
and characters show up we might need to move them around. come up with solutions that suit
the dream logic of the novel."*

**The call: the world is no longer required to be globally consistent. Stations may claim the
same ground, and what is not part of the station you are standing in may recede.**

**Why this had to be decided now.** `DIMENSIONS.md`'s finding is that we build at about one
sixtieth of the stated scale at the monuments and one twelfth in the gardens, *and that the
compression is not uniform* — which is the actual source of the crowding, not the compression
itself. Two days of work have now confirmed that the non-uniformity cannot be fixed inside one
consistent site plan:

- The **Great Portal** could be rescaled 2.3x only by moving the valley walls with it.
- The **colossus** could be doubled only because a corridor happened to be empty, and it
  stops at 1 : 3.2 because the island's eastern edge is 66 m out.
- **Polia's garden** cannot be rescaled at all. It is already the most inland point on the
  island (shore radius 48 m, against 28 at the next best place). There is no land east — the
  probes fall through to empty air past x = 75, and (75, -40) is open sea. And there is no
  free ring at ANY radius around it: 23 built objects within 5 m, 52 by 10, 30 by 15, 64 by
  20, 271 by 30. The court is built on top of the ground the garden needs.

So the choice was: leave one of the book's set-pieces permanently at 1 : 10, or stop
requiring the stations to coexist. Ted chose the second, and he is right that the book chose
it first — the *Hypnerotomachia* is a dream, the strife of love in a dream, and its spaces do
not add up. Nothing in Colonna requires the pyramid, the court and the garden to be
simultaneously true.

**This is also the project's own oldest unanswered question.** `characters/designer.md` has
carried three options for the world's structure since the beginning: A, unified geography; B,
discrete dream scenes; C, a hybrid — a coherent world with inner spaces entered through
doors, marked *"Preferred by Designer"*. `STATUS.md` records its status as **PENDING**. The
world shipped as A because A is what you get if nobody decides. This decides it: **C.**

**What it does NOT license.** Not a scene-select menu, and not cutting the walk into rooms —
the continuous world stays, and you still walk from the wood to the portal to the court
without a transition. The dream is allowed to *fold*, not to *skip*. And it changes nothing
about fidelity: the order of the book, the turnings, and the contents of each station are
still followed to the letter (2026-09-09 call 2). What folds is empty ground between
set-pieces, not events.

**The first application, and the test of it:** Polia's garden opens when the dreamer enters
it — the ivied arcade of 100 arches that Dallington p. 182 describes and that this world has
never built stands at or near its stated size, and the palace court recedes while you are
inside. Walk out and the court is there again. If that reads as a bug rather than as a dream,
the call is wrong and should be reversed.

---

## 2026-09-09 (later) — The Great Portal at scale, and the wall that was never there

*The execution of call 3 above, for the first of its three monuments. Recorded separately
because it did two things call 3 did not authorise, and one of them contradicts call 3's own
second sentence.*

**What was built.** The pyramid went from 17.5 m wide and 19.2 m tall to **40 m wide and
38.4 m tall** — 40 courses of 0.66 m where there had been 26 of 0.42, each course narrowing to
14% of the base as before, so the profile is unchanged and only the size is different. The
cube, the harpies, the socket, the obelisk and the Fortuna on top scale with it at a single
`TS = 1.8`. The whole block now stands **53.3 m** from the valley floor, measured on the
running page, against 32 before.

**The constraint call 3 flagged was real, and it was paid, not designed around.** A portal
spanning 40 m cannot stand in a valley whose cliffs converge to 44 m at the piers without the
book's absolute — *"no man could go further forward or backe againe"* (Dallington p. 27) —
becoming a polite fiction the player can walk round. So the valley moved with it:
`approach.js` `gap` from 20 to 24 and `high` from 26 to 44. **This is a change to the ground
plan, which call 3 said to leave alone.** It is a local one — a wider neck and taller walls in
the last 80 m of the approach, not a rescale of the plan — and it is the price of the
monument, not an independent wish. Call 3 stands otherwise: the mainland is still 88 × 100 m.

**And the thing the world never had: a BASE STOREY.** The pyramid had been standing on open
ground with the valley floor running past it on both sides, held shut by colliders and by the
player not thinking to try. The book does not describe a pyramid in a field; it describes a
structure that closes the valley, with the porch cut through it. So there is now an ashlar
wall 12 m tall running from each cliff to the pier of the porch, coursed at 1.3 m with 2.0 m
blocks like every other wall in the world — meaning it is made of blocks and is therefore
roll-up-able, per the 2026-09-09 call on that. The pyramid springs from its top.

**Verified, not assumed.** Walk tests from eight starting points either side, driven headless
against the real colliders: every one is stopped at the wall's face, and the only route
through is the porch down the middle. The wall is also *approachable* — an earlier fix pushed
walkers back from in front of it, which is a different bug wearing the same result. Mesh count
is unchanged at 3 788: the base storey is ashlar, and ashlar merges.

**Still open on `feat-monuments-true-scale`:** the recumbent colossus (~8 m against a stated
89) and Polia's garden (~14 m across against 141).

---

## 2026-09-09 — The reading mode, and the wolf changes hands

**Built:** `hpRead()` and `src/data/reading.json`. The whole book, page by page, with the world
moving to wherever the page happens — the inverse of every other mode, where you move and the
commentary meets you. Ted asked for it in one sentence and it needed three things finished the
same day: the translation completed, the manifest's first-half chapters refined out of their
placeholder, and Poliphilo already standing at each station in the attitude the book records.

**And on its first day it caught an error in the world.** Page 21 reads *"a famished and
flesh-eating wolf appeared to me on the **right** hand"* while the wolf stood on the left,
because `DIRECTIONS.md` had sited it from Dallington — *"I soddainely espied vpon my **left**
hand"*. The 1499 says the opposite: **"alla parte dextra"**. `DIMENSIONS.md`'s standing rule is
that where the two disagree the Italian wins and the disagreement is noted, so the wolf crossed
the valley to `x = +21`, east. The book gives left and right at only four moments in the whole
dream; this is one of them, and it decides which side of the valley the animal stands on.

**The general point, worth more than the wolf.** The world had been built on the only English
that existed, and that English is four centuries old and sometimes wrong. Now that every page
is in English beside its Italian, *the world and the text can be read against each other on one
screen* — and they will disagree again. The reading mode is not only a way to read the book; it
is the cheapest verification instrument this project has, and the first thing it did was find a
