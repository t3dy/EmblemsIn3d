<!-- tokens: ~7,392 · read for: the task table; read this first, then two or three files -->
...
**Moved to [`archive/planning-2026-06/`](archive/planning-2026-06/) on 2026-09-20** (ticket
`infra-doc-growth`, which see for the before/after token count): `INDEX.md`, `PLAN.md`,
`STATUS.md`, `HANDOFF.md`, `PROJECT_SUMMARY.md`, `COMPLETE_BRIEFING.md`, `SCENES.md`,
`VISION.md`, `TWO_WORLDS_FRAMEWORK.md`, `RESEARCH_ROADMAP.md`, `RESEARCHPASSFORVR.md` — same
filenames, new directory. `docs/*` was already off-root and is unaffected.
...
`archive/planning-2026-06/COMPLETE_BRIEFING.md` and
`archive/planning-2026-06/TWO_WORLDS_FRAMEWORK.md` still reference them by name. Both are
June 2026 planning documents from the superseded two-world framing; they are in the list
...
*Generated by `scripts/doc_costs.py`. **59 files in the root, ~326,295 tokens all told** — which is why this file exists and why you should open two or three of them and not twenty. The estimate is chars/4; it is for deciding whether you can afford a file, and being wrong by ten per cent never changes that. Re-run the script after any substantial documentation change.*

| tokens | file | read for |
|---:|---|---|
| ~136,485 | `COVERAGE.md` | what the book has that the world does not (generated) · generated |
| ~35,822 | `TICKETS.md` | the engineering queue (generated) · generated |
| ~7,999 | `NEXTSTEPS.md` | Ted's standing asks, in his words |
| ~7,392 | `ROUTER.md` | the task table; read this first, then two or three files |
| ~7,237 | `15scholars.md` | what each scholar will and will not support |
| ~6,643 | `DECISIONS.md` | the index of every directional call |
| ~5,951 | `ARCHITECTURE.md` | orders, members, the portal, the temples, the theatre |
| ~5,438 | `ROLLING.md` | every variable in Roll Up and which one to turn |
...
| ~4,545 | `DIRECTIONS.md` | where things stand in relation to each other; the compass |
| ~4,514 | `GARDENS.md` | the island's rings, terraces, parterres, topiary |
| ~4,444 | `DIMENSIONS.md` | how big anything is, in metres, with sources |
| ~4,060 | `README.md` |  |
...
| ~3,326 | `SOURCES.md` | the corpus map, and which scholar to read per asset |
| ~3,325 | `RENDERINGMODELSBLINGUSOUT.md` | the five figure rungs, the Botticelli projection, and what each costs |
| ~3,279 | `WOODCUT.md` |  |
| ~2,975 | `PROCESSIONS.md` | the triumphs, cars, teams, liveries |
| ~2,877 | `RENAISSANCEART.md` |  |
| ~2,863 | `BUILDINGPLAN.md` |  |
...
| ~2,552 | `GAMIFYVRHP.md` |  |
| ~2,525 | `HANDOVER_ROLLMODE.md` |  |
| ~2,420 | `DRAWCALLS.md` | why the world costs what it costs, and the merge we declined |
| ~2,197 | `RENDERING.md` | how the world is drawn; aerial perspective, the 1499 palette |
| ~2,145 | `HANDOVER_STAGE2.md` |  |
| ~2,066 | `CRUST.md` |  |
| ~1,942 | `CHARACTERS.md` | the named cast |
| ~1,887 | `HANDOVER.md` | where this project stands and what to do next |
...
| ~1,604 | `CLAUDE.md` | the seven rules (auto-loaded; you already have it) |
